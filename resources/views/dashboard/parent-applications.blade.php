...
<head>
    ...
    <title>My Applications - Parent Dashboard</title>
...
<body>
<!-- DEBUG: THIS IS THE REAL PARENT APPLICATIONS PAGE -->
    <script>window.__API_TOKEN = '{{ $apiToken ?? '' }}';</script>
    <div class="dash-header">
        <h1><i class="fas fa-user-circle"></i> Parent Dashboard — My Applications</h1>
        <div class="header-actions">
            <button class="header-btn primary" onclick="openAppModal()" type="button"><i class="fas fa-plus"></i> New Application</button>
            <a href="{{ url('/parent') }}" class="header-btn"><i class="fas fa-arrow-left"></i> Dashboard</a>
    ...
    <div class="dash-content">
        ...
        <div class="empty-state">
            ...
            <p>You haven't submitted any applications. Start by applying now!</p>
            ...
            <form id="appForm" class="ajax-form" action="/apply/submit" method="POST" style="padding:24px;">
                ...
                <div class="detail-grid" style="margin-bottom:14px;">
                    <div class="detail-item full">
                        ...
                        <input type="text" name="full_name" class="form-control" required placeholder="e.g., Sarah Ahmed Hassan" style="width:100%;padding:10px 12px;border-radius:8px;border:1px solid var(--border);background:var(--bg);color:var(--text);font-size:14px;">
                    </div>
                    <div class="detail-item full">
                    ...
                    <div class="detail-item full">
                        ...
                        <input type="text" name="experience" class="form-control" required placeholder="e.g., pickup_change, absence, other..." style="width:100%;padding:10px 12px;border-radius:8px;border:1px solid var(--border);background:var(--bg);color:var(--text);font-size:14px;">
                    </div>
                    <div class="detail-item full">
...
async function loadApplications() {
    ...
    try {
        const res = await fetch('/api/applications?role=parent', {
            headers: {
        ...
        console.warn('Failed to refresh parent applications', error);
    }
}
...
function viewApp(id) {
    ...
    fetch('/dashboard/parent/applications/' + id, { headers: { 'Accept': 'application/json' } })
        .then(r => r.json())
        .then(res => {
            const d = res.data;
            let meta = '';
            if (d.metadata && Object.keys(d.metadata).length) {
                meta = '<div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border)"><div class="detail-label" style="margin-bottom:10px;color:var(--accent)">Additional Details</div><div class="detail-grid">';
...
document.getElementById('appForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        this.reset();
        loadApplications();
        // Prepend new card dynamically without reload
        const app = data.data;
        if (app && document.querySelector('.app-cards')) {
            const card = document.createElement('div');
            card.className = 'app-card latest';
            card.style.borderColor = 'rgba(59,130,246,.3)';
            card.innerHTML = `
                <div class="card-main">
                    <h3>${app.full_name}</h3>
                    <div class="card-meta">
